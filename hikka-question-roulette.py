# meta developer: @your_nickname
# meta pic: none
# meta banner: https://example.com/banner.png

from .. import loader, utils
from telethon.tl.types import Message
from telethon.tl.custom import Button
import logging
import random
import asyncio

logger = logging.getLogger(__name__)

@loader.tds
class RandomQuestionGameMod(loader.Module):
    """🎮 Игра в вопросы с 6 режимами и меню выбора"""

    strings = {
        "name": "RandomQuestionGame",
        "menu": (
            "<b>🎲 Random Question Game</b>\n\n"
            "<b>Доступные режимы:</b>\n"
            "1️⃣ <b>Обычные</b> — вопросы обо всём (хобби, еда, мечты)\n"
            "2️⃣ <b>Личные</b> — секреты, страхи, сожаления\n"
            "3️⃣ <b>Против</b> — вопросы про человека напротив (кто чаще...)\n"
            "4️⃣ <b>Пикантные</b> — 18+/флирт (поцелуи, типажи, свидания)\n"
            "5️⃣ <b>Психологические</b> — про характер, эмоции, стресс\n"
            "6️⃣ <b>Социальные</b> — задания из соцсетей (селфи, сообщения)\n\n"
            "<b>Как играть:</b>\n"
            "1. Нажми на кнопку с режимом\n"
            "2. Все желающие жмут ✅ на моё сообщение\n"
            "3. Через 30 секунд я выдам вопрос"
        ),
        "waiting": "<b>🎯 Игроки, жмите ✅ в течение 30 секунд!</b>\nРежим: {}",
        "timeout": "<b>⏰ Время вышло. Игра отменена.</b>",
        "question": "<b>❓ Вопрос:</b>\n<code>{}</code>",
        "players": "<b>🎮 Игроки:</b> {}",
    }

    def __init__(self):
        self.questions = {
            "обычные": [
                "Какое твоё любимое блюдо?",
                "Что ты будешь делать, если выиграешь миллион?",
                "Какой твой самый странный страх?",
                "Какая музыка поднимает тебе настроение?",
                "Где ты мечтаешь побывать?",
                "Что ты ценишь в друзьях больше всего?",
                "Какой твой любимый фильм?",
                "Какое хобби ты хочешь попробовать?",
                "Что тебя бесит больше всего?",
                "Ты жаворонок или сова?"
            ],
            "личные": [
                "Что тебя заставляет плакать?",
                "Кому ты доверяешь больше всех?",
                "Какой у тебя самый большой секрет?",
                "Чего ты стесняешься в себе?",
                "О чём ты жалеешь больше всего?",
                "Кого из присутствующих ты знаешь дольше всех?",
                "Что бы ты изменил в своём прошлом?",
                "Ты когда-нибудь врал лучшему другу?",
                "Чего ты боишься в отношениях?",
                "Что тебя мотивирует по утрам?"
            ],
            "против": [
                "Кто из нас чаще врёт?",
                "Кто больше рискует в жизни?",
                "Кто быстрее найдёт пару?",
                "Кто чаще опаздывает?",
                "Кто лучше готовит?",
                "Кто больше зарабатывает?",
                "Кто более ленивый?",
                "Кто чаще сидит в телефоне?",
                "Кто лучше танцует?",
                "Кто громче смеётся?"
            ],
            "пикантные": [
                "С кем из присутствующих ты бы поцеловался?",
                "Какой типаж противоположного пола тебе нравится?",
                "Был ли у тебя опыт на одну ночь?",
                "Что для тебя измена?",
                "Самое смешное свидание в твоей жизни?",
                "Ты любишь флиртовать?",
                "Какая часть тела тебе нравится в людях?",
                "Что бы ты сделал, если бы остался наедине с человеком, который тебе нравится?",
                "Ты веришь в любовь с первого взгляда?",
                "Какой комплимент тебе запомнился больше всего?"
            ],
            "психо": [
                "Ты чаще слушаешь разум или сердце?",
                "Что выбесит тебя за секунду?",
                "Ты злопамятный?",
                "Как ты справляешься со стрессом?",
                "Ты доверяешь людям или проверяешь?",
                "Что для тебя идеальный отдых?",
                "Ты больше интроверт или экстраверт?",
                "Как ты миришься после ссоры?",
                "Ты чаще жалеешь о том, что сделал или не сделал?",
                "Что тебя вдохновляет?"
            ],
            "соц": [
                "Отправь случайное фото из галереи в чат",
                "Напиши 'Привет' любому контакту из списка",
                "Поставь лайк на самый старый пост у друга",
                "Отправь смешной мем в чат",
                "Напиши комплимент соседу справа",
                "Сделай селфи и отправь сюда",
                "Напиши рандомное сообщение маме",
                "Поставь статус 'Я в игре' на 10 минут",
                "Отправь голосовое с пением любимой песни",
                "Найди в интернете самый странный факт и скинь"
            ]
        }
        
        self.mode_names = {
            "обычные": "1️⃣ Обычные",
            "личные": "2️⃣ Личные",
            "против": "3️⃣ Против",
            "пикантные": "4️⃣ Пикантные",
            "психо": "5️⃣ Психологические",
            "соц": "6️⃣ Социальные"
        }

    @loader.command()
    async def gamecmd(self, message: Message):
        """Показать меню игры с кнопками"""
        # Создаем инлайн клавиатуру
        buttons = [
            [
                Button.inline("1️⃣ Обычные", data="mode_обычные"),
                Button.inline("2️⃣ Личные", data="mode_личные"),
                Button.inline("3️⃣ Против", data="mode_против"),
            ],
            [
                Button.inline("4️⃣ Пикантные", data="mode_пикантные"),
                Button.inline("5️⃣ Психологические", data="mode_психо"),
                Button.inline("6️⃣ Социальные", data="mode_соц"),
            ],
            [
                Button.inline("❓ Помощь", data="help"),
            ]
        ]
        
        await utils.answer(message, self.strings("menu"), buttons=buttons)

    async def game_handler(self, mode: str, call):
        """Обработчик игры для выбранного режима"""
        mode_display = self.mode_names.get(mode, mode)
        
        # Отправляем сообщение с ожиданием игроков
        msg = await call.edit(
            self.strings("waiting").format(mode_display),
            buttons=[Button.inline("❌ Отмена", data="cancel")]
        )
        
        try:
            # Ждём 30 секунд для сбора игроков
            await asyncio.sleep(30)
            
            # Получаем реакции
            reactions = await msg.get_reactions()
            players = []
            
            if reactions:
                for reaction in reactions:
                    if reaction.reaction == '✅':
                        for peer in reaction.peers:
                            user_id = peer.user_id
                            try:
                                user = await call.client.get_entity(user_id)
                                players.append(user.first_name or f"User{user_id}")
                            except:
                                players.append(f"User{user_id}")
            
            if len(players) < 2:
                await call.edit(self.strings("timeout"))
                return
                
            # Выбираем случайный вопрос
            question = random.choice(self.questions[mode])
            
            # Формируем ответ
            players_text = ", ".join(players)
            response = self.strings("players").format(players_text) + "\n\n" + self.strings("question").format(question)
            
            await call.edit(response)
            
        except Exception as e:
            logger.error(f"Game error: {e}")
            await call.edit(f"<b>❌ Ошибка:</b> {str(e)}")

    @loader.callback_handler()
    async def callback_handler(self, call):
        """Обработчик инлайн кнопок"""
        data = call.data.decode()
        
        if data == "help":
            await call.answer("❓ Выбери режим и нажми на кнопку", alert=True)
            return
            
        if data == "cancel":
            await call.edit("❌ Игра отменена")
            return
            
        if data.startswith("mode_"):
            mode = data[5:]  # убираем "mode_"
            await self.game_handler(mode, call)
            return
